...
import { allowedTypesMap, userFileUpload } from "../../utils/user.multerCloudinary.js"
...
//delete user
router.patch("/deleteUser",isValid(headersSchema,true),auth(endPoint.delete),userController.deleteUser)

//recover account
router.get("/accountRecovery/:reactiveToken",userController.accountRecovery)

//profile pic
router.patch("/uploadProfilePic",isValid(headersSchema,true),auth(endPoint.update),userFileUpload(allowedTypesMap).single("profile"),userController.uploadProfilePic)

//cover pic
router.patch("/uploadCoverPic",isValid(headersSchema,true),auth(endPoint.update),userFileUpload(allowedTypesMap).single("cover"),userController.uploadCoverPic)
